...
def buildMaze_with_union_by_size(cells,G,mirror=False):
    count = 0
...
def buildMaze(cells,G,mirror=False):
    count = 0
...
if m < n-1:
    ...
    if choice == 1:
        ...
        M = buildMaze_with_union_by_size(m, G,True)
        print(M)
...
if m > n-1:
    ...
    if choice == 1:
        print('building adjacency list:')
        G = [[] for i in range(n)]
        print(buildMaze_with_union_by_size(m, G,True))
        draw_maze(walls, maze_rows, maze_cols)

        M = buildMaze_with_union_by_size(m, G)
